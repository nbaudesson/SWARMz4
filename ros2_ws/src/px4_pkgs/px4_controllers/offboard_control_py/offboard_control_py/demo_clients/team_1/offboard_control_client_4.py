...
2. Detects drone 3
3. Positions itself 5m to the south of drone 3 at 1m altitude
...
class DroneController(Node):
    """
    Controller for drone 4 that positions itself 5m south of drone 3.
    ...
    def behavior_callback(self):
        ...
        elif self.state == self.STATE_FIND_DRONE3:
            self.handle_find_drone3_state()
    ...
    def check_state_timeout(self):
        ...
        if self.state == self.STATE_TAKEOFF and time_in_state > 30.0:
        ...
        elif self.state == self.STATE_FIND_DRONE3 and time_in_state > 60.0:
            self.get_logger().warning(f'Looking for drone 3 for {time_in_state:.1f}s with no detection')
            ...
            # If positioning is taking too long, we might have lost drone 3
            if time_in_state > 90.0:
                self.get_logger().warning(f'Positioning timeout after {time_in_state:.1f}s, returning to search state')
                self.change_state(self.STATE_FIND_DRONE3)
    ...
    def change_state(self, new_state):
        ...
        state_names = {
            ...
            self.STATE_FIND_DRONE3: "FIND_DRONE3",
    ...
    def handle_takeoff_state(self):
        ...
        if not self.action_in_progress:
            height = 1.0  # 1m takeoff height for drone 4
    ...
    def handle_find_drone3_state(self):
        """Handle the FIND_DRONE3 state - look for drone 3."""
        # If drone 3 is detected, the detection_callback will update drone3_detection
        if self.drone3_detection:
            self.get_logger().info('Drone 3 detected, transitioning to POSITION state')
            self.change_state(self.STATE_POSITION)
        else:
            if not hasattr(self, '_find_drone3_log_timer') or time.time() - self._find_drone3_log_timer > 5.0:
                self.get_logger().info('Scanning for drone 3...')
                self._find_drone3_log_timer = time.time()
    
    def handle_position_state(self):
        """Handle the POSITION state - move to position 5m south of drone 3."""
        # If drone 3 detection is lost, go back to find state
        if not self.drone3_detection or time.time() - self.last_detection_time > 5.0:
            self.get_logger().warning('Lost drone 3 detection, returning to FIND_DRONE3 state')
            self.change_state(self.STATE_FIND_DRONE3)
            return
            
        # Move to position south of drone 3
        if not self.action_in_progress:
            success = self.position_south_of_drone3()
    ...
    def handle_maintain_state(self):
        """Handle the MAINTAIN state - stay 5m south of drone 3."""
        # If drone 3 detection is lost, go back to find state
        if not self.drone3_detection or time.time() - self.last_detection_time > 5.0:
            self.get_logger().warning('Lost drone 3 detection, returning to FIND_DRONE3 state')
            self.change_state(self.STATE_FIND_DRONE3)
    ...
    def position_south_of_drone3(self):
        """
        Calculate and move to a position 5m south of drone 3,
        maintaining 1m altitude.
        
        Returns:
            bool: True if positioning command was sent successfully
        """
        if not self.drone3_detection or not self.estimated_position:
            self.get_logger().warning('Cannot position south of drone 3 - missing detection or position data')
            return False
        ...
        # Get drone 3's relative position (in FRD frame)
        drone3_f = self.drone3_detection.relative_position.x  # Forward
        drone3_r = self.drone3_detection.relative_position.y  # Right
        drone3_d = self.drone3_detection.relative_position.z  # Down
        ...
        rel_n = drone3_f * cos_heading - drone3_r * sin_heading
        rel_e = drone3_f * sin_heading + drone3_r * cos_heading
        
        # Calculate drone 3's absolute position in NED
        drone3_n = drone_n + rel_n
        drone3_e = drone_e + rel_e
        drone3_d = drone_d + drone3_d
        
        # Calculate position 5m south of drone 3
        # South is positive Y in NED coordinate system
        target_n = drone3_n
        target_e = drone3_e + 5.0  # 5m south in NED
        target_d = -1.0  # 1m altitude (NED frame, so negative Z)
        
        self.get_logger().info(
            f'Positioning 5m south of drone 3: '
            f'Drone 3 at ({drone3_n:.1f}, {drone3_e:.1f}, {drone3_d:.1f}), '
    ...
    def detection_callback(self, msg):
        ...
        # Find drone 3 (vehicle_type=DRONE, is_friend=True, assumed to be drone ID 3)
        drone3_found = False
        for detection in msg.detections:
            if detection.vehicle_type == Detection.DRONE and detection.is_friend:
                # We'll assume the friendly drone detected is drone 3
                # In a real implementation, we would need a more robust way to identify drone 3
                self.drone3_detection = detection
                drone3_found = True
                
                # Log drone 3 position occasionally
                if not hasattr(self, '_drone3_log_timer') or time.time() - self._drone3_log_timer > 10.0:
                    self.get_logger().info(
                        f'Drone 3 detected at relative position: '
                        f'F={detection.relative_position.x:.1f}, '
                        f'R={detection.relative_position.y:.1f}, '
                        f'D={detection.relative_position.z:.1f}'
                    )
                    self._drone3_log_timer = time.time()
                
                # If we're in FIND_DRONE3 state, move to POSITION state
                if self.state == self.STATE_FIND_DRONE3:
                    self.change_state(self.STATE_POSITION)
                
                break
        
        # If we didn't find drone 3 this time, don't immediately clear detection
        # Let the state timeout handling decide if detection is lost
        if not drone3_found and self.drone3_detection is not None:
            if time.time() - self.last_detection_time > 2.0:
                self.get_logger().warning('Lost drone 3 detection')
                self.drone3_detection = None
    ...
    def message_callback(self, msg):
        ...
        if "drone 4" in msg.data.lower() and "status" in msg.data.lower():
            if self.state == self.STATE_MAINTAIN and self.drone3_detection:
                self.send_message(
                    f"Drone 4 reporting status: Maintaining position 5m south of drone 3. "
                    f"Currently at altitude: {-self.estimated_position['z']:.1f}m"
                )
