...
class DroneController(Node):
    ...
    def position_above_ship(self):
        ...
        drone_x = self.estimated_position['x']
        drone_y = self.estimated_position['y']
        drone_z = self.estimated_position['z']
        ...
        ship_x = drone_x + rel_n
        ship_y = drone_y + rel_e
        ship_z = drone_z + ship_d
        
        # Calculate target position 2m above ship
        target_x = ship_x
        target_y = ship_y
        target_z = ship_z - 2.0  # 2m above ship in NED (negative Z is up)
        
        self.get_logger().info(
            f'Positioning 2m above ship: '
            f'Ship at ({ship_x:.1f}, {ship_y:.1f}, {ship_z:.1f}), '
            f'Moving to ({target_x:.1f}, {target_y:.1f}, {target_z:.1f})'
        )
        
        # Move to the calculated position
        self.set_offboard_parameters(offboard_mode='position', coordinate_system='NED')
        self.navigate_to(target_x, target_y, target_z, 0.0)
