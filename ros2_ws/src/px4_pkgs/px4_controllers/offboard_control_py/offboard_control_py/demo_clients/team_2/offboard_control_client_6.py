...
This module implements a controller for drone 6 that:
1. Takes off to 10m altitude
2. Navigates to position x=126, y=260, z=-10 (4m south of drone 5)
3. Stays at that position, within kamikaze explosion range of drone 5

Usage:
    ros2 run offboard_control_py offboard_control_client_6 --ros-args -r __ns:=/px4_6
"""
...
class DroneController(Node):
    """
    Controller for drone 6 that positions it 4m south of drone 5.
    ...
    def __init__(self):
        ...
        # Position drone 6 exactly 4m south of drone 5 (x=130, y=260, z=-10)
        # Since south is positive Y in NED frame, 4m south means y=264
        self.target_position = (126.0, 260.0, -10.0, 0.0)  # Target position (x, y, z, yaw)
        ...
        self.drone5_detection = None
        self.drone5_position = None
        self.kamikaze_executed = False
    ...
    def check_state_timeout(self):
        ...
        elif self.state == self.STATE_WAIT and time_in_state > 300.0:
            # Check if drone 0 has been detected in the last 5 minutes
            drone0_detected = False
            current_detection_time = self.last_detection_time or 0
            
            if current_time - current_detection_time < 300.0:
                for detection in self.latest_detections:
                    if not detection.is_friend and detection.drone_id == 0:
                        drone0_detected = True
                        break
            
            # If we've been waiting for 5 minutes with no drone 0 detection, consider mission complete
            if not drone0_detected:
                self.get_logger().info(f'Waited for {time_in_state:.1f}s without detecting drone 0, ending mission')
                self.change_state(self.STATE_COMPLETE)
            else:
                self.get_logger().info(f'Waited for {time_in_state:.1f}s, continuing to wait for drone 0')
                self.state_changed_time = current_time  # Reset timer
    ...
    def handle_takeoff_state(self):
        """Handle the TAKEOFF state behavior - take off to 10m altitude."""
    ...
    def handle_navigate_state(self):
        """Handle the NAVIGATE state behavior - go to position 4m south of drone 5."""
        if not self.action_in_progress:
    ...
    def handle_wait_state(self):
        """
        Handle the WAIT state behavior - maintain position 4m south of drone 5
        and monitor for drone 0 to perform kamikaze if necessary.
        """
        current_time = time.time()
        
        # Periodically report position
        if not hasattr(self, '_wait_log_timer') or current_time - self._wait_log_timer > 30.0:
            self.get_logger().info('Waiting 4m south of drone 5')
            self._wait_log_timer = current_time
            
            # Send status message
            self.send_message(f"Drone 6 maintaining position at x={self.target_position[0]}, "
                             f"y={self.target_position[1]}, z={self.target_position[2]}. "
                             f"Waiting for drone 0.")
        ...
        # Check if drone 5 has moved and update our position accordingly
        if self.drone5_detection:
            self.update_position_relative_to_drone5()
        
        # Check for drone 0 (enemy)
        self.check_for_drone0()
    
    def update_position_relative_to_drone5(self):
        """Update our position to stay 4m south of drone 5 if it moves."""
        # Only update occasionally and when not already moving
        current_time = time.time()
        if (self.action_in_progress or 
            not hasattr(self, '_drone5_position_update_timer') or 
            current_time - self._drone5_position_update_timer < 10.0):
            return
            
        # Calculate absolute position of drone 5 using our position and relative detection
        if not self.estimated_position:
            return
            
        # Get our current position and heading in NED
        ...
        # Get drone 5's relative position (in FRD frame)
        drone5_f = self.drone5_detection.relative_position.x  # Forward
        drone5_r = self.drone5_detection.relative_position.y  # Right
        drone5_d = self.drone5_detection.relative_position.z  # Down
        ...
        rel_n = drone5_f * cos_heading - drone5_r * sin_heading
        rel_e = drone5_f * sin_heading + drone5_r * cos_heading
        
        # Calculate drone 5's absolute position in NED
        drone5_n = drone_n + rel_n
        drone5_e = drone_e + rel_e
        drone5_d = drone_d + drone5_d
        
        # Calculate position 4m south of drone 5 (south is +ve y in NED)
        new_target_n = drone5_n
        new_target_e = drone5_e + 4.0  # 4m south in NED
        new_target_d = -10.0  # 10m altitude (NED frame, so negative Z)
        
        # Only update if position has changed significantly (more than 2m)
        position_changed = False
        old_x, old_y, old_z, _ = self.target_position
        
        if (abs(new_target_n - old_x) > 2.0 or 
            abs(new_target_e - old_y) > 2.0 or 
            abs(new_target_d - old_z) > 2.0):
            position_changed = True
            
        if position_changed:
            self.get_logger().info(
                f'Drone 5 has moved, updating our target position to: '
                f'({new_target_n:.1f}, {new_target_e:.1f}, {new_target_d:.1f})'
            )
            self.target_position = (new_target_n, new_target_e, new_target_d, 0.0)
            self.navigate_to(new_target_n, new_target_e, new_target_d, 0.0)
        
        self._drone5_position_update_timer = current_time
    
    def check_for_drone0(self):
        """Check if drone 0 is detected and respond appropriately."""
        # Only check when not executing an action
        if self.action_in_progress or not self.latest_detections:
            return
            
        # Look for drone 0
        drone0_detected = False
        drone0_distance = float('inf')
        ...
        if drone0_detected:
            # Log detection if we haven't recently
            current_time = time.time()
            if not hasattr(self, '_drone0_log_time') or current_time - self._drone0_log_time > 10.0:
                self.get_logger().info(f'Drone 0 detected at distance {drone0_distance:.1f}m')
                self._drone0_log_time = current_time
            
            # If drone 0 gets close enough, perform kamikaze attack
            if drone0_distance < 15.0 and not self.kamikaze_executed:
                self.get_logger().info(f'Drone 0 within kamikaze range at {drone0_distance:.1f}m!')
                self.send_message(f"Drone 6 executing kamikaze attack against drone 0 at {drone0_distance:.1f}m distance")
                
                # Switch to emergency state to execute kamikaze
                self.change_state(self.STATE_EMERGENCY)
    
    def handle_emergency_state(self):
        """Handle emergency actions like kamikaze attack."""
        if not self.kamikaze_executed:
            self.get_logger().info('Executing kamikaze attack against drone 0')
            success = self.execute_kamikaze()
            
            if success:
                self.kamikaze_executed = True
                # We will likely be destroyed, but set state to COMPLETE anyway
                self.change_state(self.STATE_COMPLETE)
            else:
                self.get_logger().error('Failed to execute kamikaze attack, returning to WAIT state')
    ...
    def handle_complete_state(self):
        ...
        if self.completion_phase == 'initializing':
            ...
            if self.return_to_base and not self.kamikaze_executed:
        ...
        try:
            ...
            if success:
                ...
                if self.state == self.STATE_TAKEOFF:
                    ...
                    self.change_state(self.STATE_WAIT)
                    # Send a message indicating drone 6 is in position
                    self.send_message(f"Drone 6 in position at x={self.target_position[0]}, "
                                     f"y={self.target_position[1]}, z={self.target_position[2]}. "
                                     f"4m south of drone 5.")
            else:
    ...
    def detection_callback(self, msg):
        ...
        # Process detections - look specifically for drone 5 and drone 0
        for detection in msg.detections:
            # Track drone 5 to maintain our relative position
            if detection.is_friend and detection.drone_id == 5:
                self.drone5_detection = detection
                
                # Log occasionally
                if not hasattr(self, '_drone5_log_timer') or time.time() - self._drone5_log_timer > 30.0:
                    distance = math.sqrt(
                        detection.relative_position.x**2 + 
                        detection.relative_position.y**2 + 
                        detection.relative_position.z**2
                    ...
                    self.get_logger().info(
                        f'Drone 5 detected at distance {distance:.1f}m, '
                    ...
                    self._drone5_log_timer = time.time()
            
            # Track drone 0 (enemy) for potential kamikaze attack
            elif not detection.is_friend and detection.drone_id == 0:
                distance = math.sqrt(
                    detection.relative_position.x**2 + 
                    detection.relative_position.y**2 + 
                    detection.relative_position.z**2
                ...
                # Log occasionally
                if not hasattr(self, '_drone0_detect_log_timer') or time.time() - self._drone0_detect_log_timer > 30.0:
                    self.get_logger().info(f'Drone 0 detected at distance {distance:.1f}m')
                    self._drone0_detect_log_timer = time.time()
                
                # If drone 0 gets close and we're in WAIT state, consider kamikaze
                if distance < 15.0 and self.state == self.STATE_WAIT and not self.kamikaze_executed:
                    self.get_logger().info(f'Drone 0 within kamikaze range at {distance:.1f}m!')
                    self.send_message(f"Drone 6 detecting drone 0 within kamikaze range at {distance:.1f}m")
                    # Considered triggering kamikaze here, but better to let the behavior_callback handle it
    ...
    def health_callback(self, msg):
        ...
        if self.health < old_health:
            ...
            if self.health < 50 and old_health >= 50:
                self.send_message(f"Drone 6 damaged! Health at {self.health}%")
                
            # If health drops below 30, consider mission complete to avoid destruction
            if self.health < 30 and not self.kamikaze_executed:
                self.get_logger().warning('Health critically low, attempting kamikaze as last resort')
                # Try kamikaze if drone 0 is detected, otherwise complete mission
                drone0_detected = False
                for detection in self.latest_detections:
                    if not detection.is_friend and detection.drone_id == 0:
                        drone0_detected = True
            ...
            if self.health == 0:
                self.get_logger().error("Drone 6 has been destroyed!")
    ...
    def message_callback(self, msg):
        ...
        # Monitor for messages from drone 5
        if "Drone 5" in msg.data:
            # If drone 5 reports it's in position, confirm our own position
            if "in position" in msg.data:
                self.get_logger().info("Drone 5 reports it's in position, checking our position...")
                
                # If we're not in WAIT state, try to get into position
                if self.state != self.STATE_WAIT and self.state != self.STATE_EMERGENCY:
                    self.get_logger().info("Not in position yet, moving to target coordinates")
                    x, y, z, yaw = self.target_position
                    self.navigate_to(x, y, z, yaw)
            
            # If drone 5 reports damage, increase vigilance
            if "damaged" in msg.data.lower():
                self.get_logger().warning("Drone 5 reports damage, increasing detection frequency")
                # Could adjust the position update frequency here
        
        # If someone asks about drone 0, share information
        if "drone 0" in msg.data.lower() and "where" in msg.data.lower():
            drone0_detected = False
            for detection in self.latest_detections:
                if not detection.is_friend and detection.drone_id == 0:
                    drone0_detected = True
                    ...
                    self.send_message(
                        f"Drone 6 reporting: I see drone 0 at distance {distance:.1f}m, "
                        f"relative position: F={detection.relative_position.x:.1f}, "
                        f"R={detection.relative_position.y:.1f}, "
                        f"D={detection.relative_position.z:.1f}"
                    )
            ...
            if not drone0_detected:
                self.send_message("Drone 6 reporting: I don't currently see drone 0")
