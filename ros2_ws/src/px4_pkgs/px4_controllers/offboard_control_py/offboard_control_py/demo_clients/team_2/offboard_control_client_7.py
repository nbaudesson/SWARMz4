...
This module implements a controller for drone 7 that:
1. Takes off to 10m altitude
2. Navigates to position x=122.75, y=263.2, z=-10
3. Stays at that position

Usage:
    ros2 run offboard_control_py offboard_control_client_7 --ros-args -r __ns:=/px4_7
"""
...
class DroneController(Node):
    """
    Controller for drone 7 that positions it at a specific location.
    ...
    def __init__(self):
        ...
        self.target_position = (122.75, 263.2, -10.0, 0.0)  # x, y, z, yaw
    ...
    def check_state_timeout(self):
        ...
        if self.state == self.STATE_INIT and time_in_state > 30.0:
            ...
            # Just log that we've been waiting a long time
            self.get_logger().info(f'Waited at position for {time_in_state:.1f}s')
            self.state_changed_time = current_time  # Reset timer to avoid spam
    ...
    def handle_wait_state(self):
        """Handle the WAIT state behavior - stay at target position."""
        current_time = time.time()
        ...
        if not hasattr(self, '_wait_log_timer') or current_time - self._wait_log_timer > 30.0:
            self.get_logger().info(f'Waiting at position: x={self.target_position[0]}, y={self.target_position[1]}, z={self.target_position[2]}')
            self._wait_log_timer = current_time
            
            # Send a status message
            self.send_message(f"Drone 7 maintaining position at x={self.target_position[0]}, "
        ...
        if enemy_count > 0 and (not hasattr(self, '_enemy_log_time') or current_time - self._enemy_log_time > 30.0):
            ...
            if closest_enemy:
                ...
                if min_distance < 50.0:
                    self.send_message(
                        f"Drone 7 reporting: Enemy drone detected at {min_distance:.1f}m distance. "
        ...
        try:
            ...
            if success:
                ...
                if self.state == self.STATE_TAKEOFF:
                    ...
                    # Send a message indicating drone 7 is in position
                    self.send_message(f"Drone 7 in position at x={self.target_position[0]}, y={self.target_position[1]}, z={self.target_position[2]}.")
            else:
    ...
    def health_callback(self, msg):
        ...
        if self.health < old_health:
            ...
            if self.health < 50 and old_health >= 50:
                self.send_message(f"Drone 7 damaged! Health at {self.health}%")
            ...
            if self.health == 0:
                self.get_logger().error("Drone 7 has been destroyed!")
    ...
    def message_callback(self, msg):
        ...
        if "drone 7" in msg.data.lower() and ("position" in msg.data.lower() or "where" in msg.data.lower()):
            if self.estimated_position:
                response = (
                    f"Drone 7 reporting position: x={self.estimated_position['x']:.1f}, "
