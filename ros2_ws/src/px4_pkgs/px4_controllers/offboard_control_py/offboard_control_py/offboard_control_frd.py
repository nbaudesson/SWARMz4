...
class OffboardControlFRD(Node):
    ...
    def execute_navigation(self, goal_handle: ServerGoalHandle, start_time, timeout, feedback_msg):
        ...
        while rclpy.ok():
            try:
                ...
                feedback_msg.current_position.position.x = self._current_pos[0]
                feedback_msg.current_position.position.y = self._current_pos[1]
                feedback_msg.current_position.position.z = self._current_pos[2]
    ...
    def calculate_timeout(self, target_position) -> float:
        ...
        # Calculate distance directly without spawn position offset
        target_pos = [target_position.x, target_position.y, target_position.z]
        distance = self.calculate_distance(self._current_pos, target_pos)
