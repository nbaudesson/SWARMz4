...
class DroneController(Node):
    ...
    Simple implementation focused on reliability.
    """
    
    # State machine states - simplified to essential states
    STATE_INIT = 0
    STATE_TAKEOFF = 1
    STATE_NAVIGATE = 2
    STATE_WAIT = 3
    ...
    def handle_navigate_state(self):
        """Handle the NAVIGATE state behavior - go to position (126, 260, -10)."""
        if not self.action_in_progress:
    ...
    def health_callback(self, msg):
        ...
        # Only log if health changed, but don't change behavior
        if old_health != self.health:
            self.get_logger().info(f'Health changed: {old_health} -> {self.health}')
            # No need to do anything specific for health changes
    ...
    def message_callback(self, msg):
        ...
        # If someone asks about drone 6, respond with our position
        if f"drone {self.drone_id}" in msg.data.lower() and ("position" in msg.data.lower() or "where" in msg.data.lower()):
            self.send_message(f"Drone {self.drone_id} reporting position: "
                             f"x={self.target_position[0]}, y={self.target_position[1]}, z={self.target_position[2]}")
