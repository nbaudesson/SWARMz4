...
class OffboardControlGoto(Node):
    """Node for controlling a vehicle in offboard mode to go to specific coordinates."""
    ...
    def timer_callback(self) -> None:
        ...
        if self.offboard_setpoint_counter < 20:
            # Takeoff to a safe height before moving to the target coordinates
            self.publish_position_setpoint(0.0, 0.0, self.target_z)
            self.get_logger().info(f'Takeoff setpoint: [0.0, 0.0, {self.target_z}]')
        elif self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
            self.publish_position_setpoint(self.target_x, self.target_y, self.target_z)
            self.get_logger().info(f'Navigating to target setpoint: [{self.target_x}, {self.target_y}, {self.target_z}]')
...
def main(args=None) -> None:
    ...
    offboard_control_goto = OffboardControlGoto()
    rclpy.spin(offboard_control_goto)
    offboard_control_goto.destroy_node()
    rclpy.shutdown()
