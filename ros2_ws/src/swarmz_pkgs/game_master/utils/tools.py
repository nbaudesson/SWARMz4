...
from gazebo_subscriber import GazeboPosesTracker
...
def get_all_robots(node):
    ...
    """
    from ros2cli.node.direct import get_node_names_and_namespaces
    _, all_nodes = get_node_names_and_namespaces(node.get_namespace())
    return [name for name in all_nodes if 'robot' in name]

def get_all_namespaces(node):
    """
    Get a list of all unique namespaces in the ROS 2 system.
    :param node: The rclpy node
    :return: List of unique namespaces
    """
    from ros2cli.node.direct import get_node_names_and_namespaces
    _, all_nodes = get_node_names_and_namespaces(node.get_namespace())
    namespaces = set(namespace for _, namespace in all_nodes)
    return list(namespaces)

def get_all_drones(node):
    """
    Get all PX4 drones running in the ROS 2 system.
    :param node: The rclpy node
    :return: List of PX4 drone names
    """
    from ros2cli.node.direct import get_node_names_and_namespaces
    _, all_nodes = get_node_names_and_namespaces(node.get_namespace())
    return [name for name in all_nodes if 'px4' in name]


def get_all_ships(node):
    """
    Get all ship robots running in the ROS 2 system.
    :param node: The rclpy node
    :return: List of ship robot names
    """
    from ros2cli.node.direct import get_node_names_and_namespaces
    _, all_nodes = get_node_names_and_namespaces(node.get_namespace())
    return [name for name in all_nodes if 'ship' in name]

def get_relative_position(shooter_position, shooter_orientation, target_position):
    """
    Transform the target position to the shooter's reference frame.
...
def is_aligned(node, shooter_position, shooter_orientation, target_position, target_padding, threshold, verbose=False):
    ...
    try:
        ...
        relative_position = get_relative_position(shooter_position, shooter_orientation, target_position)
        ...
        projected_corners = [get_relative_position(shooter_position, shooter_orientation, corner) for corner in bounding_box_corners]
...
def get_robot_position(node, ns):
    """
